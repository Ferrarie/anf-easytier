...
mod tests {
    ...
    fn uses_default_subnet_when_routes_have_no_ipv4() {
        ...
        assert_eq!(
            ...
            DhcpIpv4Decision::Change {
                previous: None,
                next: Some("10.200.126.1/24".parse().unwrap()),
            }
        );
    }
    ...
    async fn service_commits_only_after_host_apply_succeeds() {
        ...
        assert_eq!(service.current(), Some("10.200.126.1/24".parse().unwrap()));
        assert_eq!(
            *host.changes.lock().unwrap(),
            [(None, Some("10.200.126.1/24".parse().unwrap()))]
        );
        assert_eq!(
            runtime_config.snapshot().peer.runtime.core.routes.ipv4,
            Some(IpPrefix::new("10.200.126.1".parse().unwrap(), 24).unwrap())
        );
        assert_eq!(
            *host.published.lock().unwrap(),
            [(
                None,
                Some("10.200.126.1/24".parse().unwrap()),
                Some("10.200.126.1/24".parse().unwrap())
            )]
        );
    }
    ...
    async fn service_holds_host_permit_through_store_and_event_commit() {
        ...
        let expected = Some(IpPrefix::new("10.200.126.1".parse().unwrap(), 24).unwrap());
