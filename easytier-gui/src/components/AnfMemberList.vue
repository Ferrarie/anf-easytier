  ...
  <div class="mt-3">
    <DataTable :value="rows" striped-rows size="small" :loading="loading" class="w-full text-sm">
      ...
      <Column field="hostname" header="成员名" sortable>
        <template #body="{ data }">
          <div class="font-medium">
            {{ friendlyHostname(data.hostname) }}
          </div>
      ...
      <Column header="方式" sortable field="cost" style="width: 5rem">
        <template #body="{ data }">
          <Tag :severity="costSeverity(data.cost)">
            {{ costLabel(data.cost) }}
          </Tag>
        </template>
      </Column>
      ...
      <Column field="lat_ms" header="延迟ms" sortable style="width: 6rem">
        <template #body="{ data }">
          {{ data.lat_ms >= 0 ? data.lat_ms : '-' }}
        </template>
      </Column>
      <Column field="loss_rate" header="丢包率" sortable style="width: 6rem">
        <template #body="{ data }">
          {{ data.loss_rate ? `${(data.loss_rate * 100).toFixed(2)}%` : '0%' }}
        </template>
      </Column>
      ...
      <Column header="接收" sortable field="rx_bytes" style="width: 8rem">
        <template #body="{ data }">
          {{ fmtBytes(data.rx_bytes) }}
        </template>
      </Column>
      <Column header="传输" sortable field="tx_bytes" style="width: 8rem">
        <template #body="{ data }">
          {{ fmtBytes(data.tx_bytes) }}
        </template>
      </Column>
    </DataTable>
    <Message v-if="error" severity="warn" :closable="false" class="mt-2 m-0">
      {{ error }}
    </Message>
  </div>
</template>
