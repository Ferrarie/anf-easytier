...
function onSaveConfigs(event: Event<StoredGuiConfig[]>) {
  console.warn(`Received event '${EVENTS.SAVE_CONFIGS}': ${event.payload}`)
...
async function onPreRunNetworkInstance(event: Event<unknown>) {
  ...
  console.warn(`Received event '${EVENTS.PRE_RUN_NETWORK_INSTANCE}', raw payload:`, event.payload, 'normalized:', instanceId)
  if (type() === 'android') {
...
async function onPostRunNetworkInstance(event: Event<unknown>) {
  const instanceId = normalizeInstanceIdPayload(event.payload)
  console.warn(`Received event '${EVENTS.POST_RUN_NETWORK_INSTANCE}', raw payload:`, event.payload, 'normalized:', instanceId)
  if (type() === 'android') {
...
async function onVpnServiceStop(event: Event<unknown>) {
  console.warn(`Received event '${EVENTS.VPN_SERVICE_STOP}', raw payload:`, event.payload)
  await syncMobileVpnService()
}

async function onDhcpIpChanged(event: Event<unknown>) {
  const instanceId = normalizeInstanceIdPayload(event.payload)
  console.warn(`Received event '${EVENTS.DHCP_IP_CHANGED}' for instance: ${instanceId}`)
  if (type() === 'android') {
...
async function onProxyCidrsUpdated(event: Event<unknown>) {
  const instanceId = normalizeInstanceIdPayload(event.payload)
  console.warn(`Received event '${EVENTS.PROXY_CIDRS_UPDATED}' for instance: ${instanceId}`)
  if (type() === 'android') {
    await onNetworkInstanceUpdate(instanceId)
  }
}
